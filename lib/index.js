 ...
 * {description}
 * @module {name}
 * @param opts {Object} - optional options. Options to be passed to the task function should be provided in this object.
 * @param {Object} [opts.oneTimeRun=false] - flag to run the task only once no matter how many data chunks are passed through the stream
 ...
 * @param task {Function} - the asynchronous task to call and wait for callback to be executed.
 ...
 * @param done {Function} - the callback function called once the asynchronous task has completed.
 * The function must have the following signature: done(error, data).
 * @returns {readable-stream/transform}
 * @example
 {>example-index/}
 */
module.exports = function (opts, task, done) {
    //check options
    opts = opts || {};
    _.defaults(opts, {
        "oneTimeRun": false,
    ...
    return through2.obj(function (chunk, enc, cb) {
        ...
        if (!opts.oneTimeRun || (opts.oneTimeRun && !processed)) {
            processed = true;
            task(opts, chunk, function (error, data) {
                done(error, data);
